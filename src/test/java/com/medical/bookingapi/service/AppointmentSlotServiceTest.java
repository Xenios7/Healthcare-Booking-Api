...
class AppointmentSlotServiceTest {
  ...
  void setup() {
    ...
    dto1 = new AppointmentSlotDTO(); // use setters/record if you have them
  }
  ...
  void createSlot_mapsEntity_setsDoctor_unbooks_andSaves() {
    ...
    trySet(create, "startTime", LocalDateTime.of(2025,1,22,14,0));
    trySet(create, "endTime",   LocalDateTime.of(2025,1,22,14,30));
    ...
    when(slotMapper.toDto(any(AppointmentSlot.class))).thenReturn(dto1);
  ...
  void updateSlot_happyPath_updatesFields_andSaves() {
    when(slotRepo.findById(100L)).thenReturn(Optional.of(slot1)); // slot1 is not booked
  ...
  void updateSlot_throwsWhenBookedAndNotUnbooking() {
      when(slotRepo.findById(101L)).thenReturn(Optional.of(slot2)); // slot2 is booked=true

      AppointmentSlotDTO patch = new AppointmentSlotDTO();
      patch.setBooked(true); // directly set booked -> true
  ...
  void updateSlot_allowsUnbookingBookedSlot() {
    when(slotRepo.findById(101L)).thenReturn(Optional.of(slot2)); // booked=true
    when(slotRepo.save(slot2)).thenReturn(slot2);
    when(slotMapper.toDto(slot2)).thenReturn(dto1);

    AppointmentSlotDTO patch = new AppointmentSlotDTO();
    trySet(patch, "booked", false); // unbooking is allowed
